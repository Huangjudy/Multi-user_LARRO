...
namespace Microsoft.MixedReality.WebRTC.Unity
{
    ...
    public class PeerConnection : WorkQueue, ISerializationCallbackReceiver
    {
        ...
        /// Need notify
        public void SendControlData(float controlcolor)
        {
            if (_CameraChannel.State == DataChannel.ChannelState.Open)
            {
                ...
                var start_time = DateTime.Now.ToString("ss.ffffff");
                string track_data = position_x + ',' + position_y + ',' + position_z + ',' + rotation_yoll + ',' + rotation_pitch + ',' + rotation_raw + ',' + start_time + ',' + controlcolor;
                byte[] textposition = System.Text.Encoding.UTF8.GetBytes(track_data);
                _CameraChannel.SendMessage(textposition);

            }
            //Debug.Log(track_data);
        }
        ...
        private void OnPostInitialize()
        {
            ...
            _nativePeer.AddDataChannelAsync(CameraChannelID, "position", true, true).ContinueWith((prevTask) =>
            {
                ...
                var CameraChannel = prevTask.Result;
                //_CameraChannel = CameraChannel;
                //Debug.Log("DataChannel for Position created successfully.");

                InvokeOnAppThread(() =>
                {
                    _CameraChannel = CameraChannel;
                    _CameraChannel.MessageReceived += ChatMessageReceived;
                    _CameraChannel.StateChanged += Channel_StateChanged;
                    Debug.Log("DataChannel for Position created successfully.");
                });
            ...
            _nativePeer.AddDataChannelAsync(LatencyChannelID, "Latency", true, true).ContinueWith((prevTask) =>
            {
                ...
                {
                    _LatencyChannel = LatencyChannel;
                    //_LatencyChannel.MessageReceived += ChatMessageReceived;
                    //_LatencyChannel.StateChanged += Channel_StateChanged;
                    Debug.Log("DataChannel for Mesh created successfully.");
                });
            ...
            _nativePeer.AddDataChannelAsync(SendtoClientChannelID, "SendtoClient", true, true).ContinueWith((prevTask) => // 用來接收Server端傳送的資料

            {
                ...
                InvokeOnAppThread(() =>
                {
                    _SendtoClientChannel.MessageReceived += ReceivedFromServer;
                });

                Debug.Log("DataChannel for Received created successfully.");
            });

        }
        public void UseDataChannel(string channelname, string data)
        {
            if (channelname == "Camera")  // channelID --> 1
            {
                if (_CameraChannel.State == DataChannel.ChannelState.Open)
                {
                    byte[] decodedText = System.Text.Encoding.UTF8.GetBytes(data);
                    _CameraChannel.SendMessage(decodedText);
                }
                else
                {
                    Debug.Log($"{_CameraChannel.State}");
                }
            }

            if (channelname == "Latency")  // channelID --> 2
            {
                if (_LatencyChannel.State == DataChannel.ChannelState.Open)
                {
                    byte[] decodedText = System.Text.Encoding.UTF8.GetBytes(data);
                    _LatencyChannel.SendMessage(decodedText);
        ...
        private void Channel_StateChanged()
        {
            ...
            foreach (var channel in _nativePeer.DataChannels)
            {
        ...
        //new
        private string _Receivedfromserver = "";
        private string _Interact_data_json = "";
        private void ReceivedFromServer(byte[] message)
        {
            _Receivedfromserver = System.Text.Encoding.UTF8.GetString(message);
        }
        private void InteractReceived(byte[] message)
        {
            _Interact_data_json = System.Text.Encoding.UTF8.GetString(message);
        }
        public string GetInteractivedata()
        {
            return _Interact_data_json;
        }
        public string GetReceivedDataFromServer()
        {
