...
namespace Microsoft.MixedReality.WebRTC.Unity
{
    ...
    public class PeerConnection : WorkQueue, ISerializationCallbackReceiver
    {
        ...
        // 在連線後創建Datachannel所需的變數
        private DataChannel _CameraChannel = null;
        private DataChannel _LatencyChannel = null;
        private DataChannel _InteractChannel = null;
        private DataChannel _SendPositiontoClientChannel = null;
        //private DataChannel _SendtoClientChannel = null;
        private const int CameraChannelID = 1;
        private const int LatencyChannelID = 2;
        private const int InteractChannelID = 3;
        //private const int SendtoClientChannelID = 4;
        private const int SendPositiontoClientChannelID = 5;
        ...
        public void UseDataChannel(string channelname, string data)
        {
            if (channelname == "SendPositiontoClient")  // channelID --> 5
            {
                if (_SendPositiontoClientChannel.State == DataChannel.ChannelState.Open)
                {
                    byte[] decodedText = System.Text.Encoding.UTF8.GetBytes(data);
                    _SendPositiontoClientChannel.SendMessage(decodedText);
                }
            }
            /*
            if (channelname == "SendtoClient")  // channelID --> 4
            {
                if (_SendtoClientChannel.State == DataChannel.ChannelState.Open)
                {
                    byte[] decodedText = System.Text.Encoding.UTF8.GetBytes(data);
                    _SendtoClientChannel.SendMessage(decodedText);
                }
            }
            */
            if (channelname == "interact")  // channelID --> 3
            {
                if (_InteractChannel.State == DataChannel.ChannelState.Open)
                {
                    byte[] decodedText = System.Text.Encoding.UTF8.GetBytes(data);
                    _InteractChannel.SendMessage(decodedText);
                }
            }
        ...
        private void OnPostInitialize()
        {
            ...
            init = true;//判斷initialized successfully
            ...
            {
                // Build_DataChannel
                _nativePeer.AddDataChannelAsync(CameraChannelID, "position", true, true).ContinueWith((prevTask) =>
                {
                    if (prevTask.Exception != null)
                    {
                        throw prevTask.Exception;
                    }
                    var newDataChannel = prevTask.Result;// 數據通道透過addDataChannelAsync建立後的結果
                    _CameraChannel = newDataChannel;
                    InvokeOnAppThread(() =>// 將資料丟回主線程執行
                    {

                        _CameraChannel.MessageReceived += CameraReceived;   // receive here
                    });
                    Debug.Log("DataChannel for Position created successfully.");
                });

                _nativePeer.AddDataChannelAsync(LatencyChannelID, "Latency", true, true).ContinueWith((prevTask) =>
                {
                    if (prevTask.Exception != null)
                    {
                        throw prevTask.Exception;
                    }
                    var LatencyChannel = prevTask.Result;
                    _LatencyChannel = LatencyChannel;

                    InvokeOnAppThread(() =>
                    {
                        _LatencyChannel.MessageReceived += LatencyReceived;
                    });

                    Debug.Log("DataChannel for Latency created successfully.");
                });

                _nativePeer.AddDataChannelAsync(InteractChannelID, "interact", true, true).ContinueWith((prevTask) =>
                {
                    if (prevTask.Exception != null)
                    {
                        throw prevTask.Exception;
                    }
                    var InteractChannel = prevTask.Result;
                    _InteractChannel = InteractChannel;
                    InvokeOnAppThread(() =>
                    {

                        _InteractChannel.MessageReceived += InteractReceived;
                        //_InteractChannel.StateChanged += Channel_StateChanged;
                        Debug.Log("DataChannel for Interact created successfully.");
                    });
                });
                _nativePeer.AddDataChannelAsync(SendPositiontoClientChannelID, "SendPositiontoClient", true, true).ContinueWith((prevTask) =>
                {
                    if (prevTask.Exception != null)
                    {
                        throw prevTask.Exception;
                    }
                    var SendPositiontoClientChannel = prevTask.Result;

                    _SendPositiontoClientChannel = SendPositiontoClientChannel;

                    Debug.Log("DataChannel for SendPositiontoClient created successfully.");
                });
                /*
                _nativePeer.AddDataChannelAsync(SendtoClientChannelID, "SendtoClient", true, true).ContinueWith((prevTask) =>
                {
                    if (prevTask.Exception != null)
                    {
                        throw prevTask.Exception;
                    }
                    var SendtoClientChannel = prevTask.Result;

                    _SendtoClientChannel = SendtoClientChannel;

                    Debug.Log("DataChannel for SendtoClient created successfully.");
                });
                */
            }
        }

        private string Mesh_data_json;
        private string Camera_data_json = "";
        private string Interact_data_json = "";
        private string Correction_data_json = "";
        private string Received_data_time = "";
        private string Latency_data_json = "";

        private void CameraReceived(byte[] message)
        {
            Received_data_time = DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss.fffffff");
            Camera_data_json = System.Text.Encoding.UTF8.GetString(message);
            //Debug.Log(Camera_data_json);
        }
        private void MeshReceived(byte[] message)
        {
            Mesh_data_json = System.Text.Encoding.UTF8.GetString(message);
            Debug.Log(Mesh_data_json);
            InvokeOnAppThread(() => GenerateMesh(Mesh_data_json));
        }
        private void InteractReceived(byte[] message)
        {
            Interact_data_json = System.Text.Encoding.UTF8.GetString(message);
        }
        private void LatencyReceived(byte[] message)
        {
            Latency_data_json = System.Text.Encoding.UTF8.GetString(message);
        }
        ...
        public void GenerateMesh(string Meshdata)
        {

            GameObject SpatialMeshObject = GameObject.Find("SpatialMesh");
            Material material = SpatialMeshObject.GetComponent<MeshRenderer>().material;
            string[] subMeshdata = Meshdata.Split('+');

            List<float[]> vertices_data = JsonConvert.DeserializeObject<List<float[]>>(subMeshdata[0]);
            int[] triangle = JsonConvert.DeserializeObject<int[]>(subMeshdata[1]);

            int vertex_number = vertices_data.Count;
            Vector3[] vertices = new Vector3[vertex_number];

            for (int i = 0; i < vertex_number; i++)
            {
                vertices[i] = new Vector3(vertices_data[i][0], vertices_data[i][1], vertices_data[i][2]);
            }
            ...
            string Mesh_name = Guid.NewGuid().ToString();

            GameObject MeshObject = new GameObject(Mesh_name, typeof(MeshFilter), typeof(MeshRenderer));

            MeshObject.transform.parent = SpatialMeshObject.transform;
